...
// I'm treating this file as roughly "typechecked JSON" and don't need to handle failures on constants that don't occur
...
fn year(y: i32) -> UncertainDate {
    UncertainDate::with_precision(
        NaiveDate::from_ymd_opt(y, 1, 1).expect("fixture month/day always valid"),
        DatePrecision::Year,
    )
    .expect("year precision is always valid")
}

fn exact(y: i32, m: u32, d: u32) -> UncertainDate {
    UncertainDate::with_precision(
        NaiveDate::from_ymd_opt(y, m, d).expect("fixture date always valid"),
        DatePrecision::Day,
    )
    .expect("day precision is always valid")
}

fn range(y1: i32, y2: i32) -> UncertainDate {
    UncertainDate::bounded(
        Some(
            DateBound::new(
                NaiveDate::from_ymd_opt(y1, 1, 1).expect("fixture month/day always valid"),
                DatePrecision::Year,
            )
            .expect("year precision is always valid"),
        ),
        Some(
            DateBound::new(
                NaiveDate::from_ymd_opt(y2, 1, 1).expect("fixture month/day always valid"),
                DatePrecision::Year,
            )
            .expect("year precision is always valid"),
        ),
...
fn lang(tag: &str) -> LanguageTag<String> {
...
fn coords(lat: f64, lon: f64, radius_m: Option<f64>) -> UnresolvedLocation {
    let location = if let Some(r) = radius_m {
        ...
        Location::point(lat, lon).expect("fixture coordinates always valid")
    };
    UnresolvedLocation::Resolved(location)
}
...
fn penn_station_bundle() -> TestBundle {
    TestBundle {
...
fn palace_of_fine_arts_bundle() -> TestBundle {
    TestBundle {
        entities: BTreeMap::from([
...
fn berlin_wall_bundle() -> TestBundle {
    TestBundle {
        entities: BTreeMap::from([
...
fn jrk_canal_bundle() -> TestBundle {
    TestBundle {
        entities: BTreeMap::from([
            ("jrk_canal", jrk_canal()),
...
pub(crate) fn all_bundles() -> Vec<(&'static str, TestBundle)> {
    let mut bundles = vec![
...
mod tests {
    ...
    #[test]
    fn all_bundles_pass_consistency_checks() -> Result<(), String> {
