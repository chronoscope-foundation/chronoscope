...
/// [`crate::grammar::event::Fact::DurationalDate`] with a [`DurationalRole`].
...
pub enum DurationalKind {
    ...
    /// Carries a damage cause via [`crate::grammar::event::Fact::DamageCause`].
    Damaged,
    /// Repair work after damage.
    Repaired,
    /// Physical relocation of the structure. Carries the method via
    /// [`crate::grammar::event::Fact::MoveMethod`].
    Moved,
}

/// A point lifetime event — one that describes a single instant. Its facts use
/// [`crate::grammar::event::Fact::PointDate`] and carry no [`DurationalRole`].
#[derive(
    Debug,
    Clone,
    Copy,
...
/// facts on [`crate::grammar::assertions::FactualAssertion`], not as values inside
/// an event reference. Once-ness for the bookends is structural rather than
/// rule-enforced.
///
/// An event declares its kind through a
/// [`crate::grammar::event::Fact::HasEvent`] claim. The submit layer requires each
