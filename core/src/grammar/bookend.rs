...
//! [`crate::grammar::event::Fact::MovedToLocation`], falling back to
...
//! outer-variant scopes on [`crate::grammar::assertions::FactualAssertion`] — an
...
//!   [`crate::grammar::event::Fact`] keyed on the entity, and must precede
//!   `Demolition::Started`.
//! - `Demolition::Started` must follow every interior
//!   [`crate::grammar::event::Fact`] keyed on the entity. Date evidence that
...
//!   [`crate::grammar::event::Fact::MovedToLocation`] on a `Moved` event overrides
//!   it.
