...
pub(super) fn retract_commit_fact(
    target: crate::grammar::ids::CommitId,
) -> Result<SubmitFact, Box<dyn std::error::Error>> {
    Ok(SubmitFact::Meta {
...
fn retract_fact(
    target: crate::grammar::ids::FactId,
) -> Result<SubmitFact, Box<dyn std::error::Error>> {
    Ok(SubmitFact::Meta {
...
fn supersede_fact(
    target: crate::grammar::ids::FactId,
    replacement: crate::grammar::ids::FactId,
) -> Result<SubmitFact, Box<dyn std::error::Error>> {
    Ok(SubmitFact::Meta {
...
async fn retract_commit_targeting_unrecorded_commit_rejected() -> TestResult {
    let store = MemoryFactStore::new();
    let phantom = crate::grammar::ids::CommitId::parse("0".repeat(64))?;
...
async fn read_snapshot_placement_never_inflight_past_watermark() -> TestResult {
    ...
    assert_eq!(
        view.placement(crate::grammar::ids::FactId::new(500))
            .await
            .map_err(|e| format!("{e:?}"))?,
        FactPlacement::Absent
    );
    ...
    assert_eq!(
        view.placement(crate::grammar::ids::FactId::new(0))
            .await
            .map_err(|e| format!("{e:?}"))?,
        FactPlacement::Committed
    );
...
async fn cross_source_kind_conflict_via_same_event_stored() -> TestResult {
    ...
    let c2: TestBundle = SubmitBundle {
        ...
        facts: [
            ...
            SubmitFact::Judgment {
                assertion: JudgmentAssertion::Identity {
                    fact: crate::grammar::identity::Fact::same_event(EventIdx(0), EventIdx(1))?,
                },
                citation: judgment_citation()?,
            },
        ]
