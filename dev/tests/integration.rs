...
/// Test that loading an ingestion bundle feeds images into the worker pipeline.
///
/// This verifies the end-to-end integration between `load_bundle()` and the
/// existing fetch→analyze pipeline:
/// 1. Load a bundle with an entity and an image URL (reusing a Reddit gallery fixture)
/// 2. Workers claim the image from the URL queue (generic affinity)
/// 3. Worker fetches the image (served from VCR fixture)
/// 4. Analysis worker processes it through Triton (served from analysis fixture)
///
...
async fn test_bundle_loaded_images_flow_through_worker_pipeline() -> TestResult {
    ...
    // Image URL from the Reddit gallery fixture (same one used in test_reddit_gallery_end_to_end).
    // Has both HTTP fixture (1.1MB JPEG, 3504x2336) and Triton analysis fixture.
    let image_url = "https://preview.redd.it/yqu901j9jddg1.jpg?width=3504&format=pjpg&auto=webp&s=3811410cf9613a933d5d958ab56f2c049d94c2cc";

    let e0 = EntityIdx::new(0);
    let s0 = SourceIdx::new(0);
        ...
        external_links: BTreeMap::from([(
            ...
            ExternalLink {
                target: LinkTarget::Wikidata {
                    entity_id: WikidataEntityId("Q99999".to_string()),
                },
                link_type: LinkType::SameAs,
            },
        )]),
        entity_links: BTreeMap::from([(e0, vec![l0])]),
    ...
    // Load bundle directly into the database (bypassing HTTP API)
    let load_result =
        chronoscope_db::ingestion::load_bundle(server.server.db().pool_ref(), &bundle).await?;

    assert_eq!(load_result.entities_created, 1);
    ...
    let (width, height) = media_row.ok_or("image should be stored as media")?;
